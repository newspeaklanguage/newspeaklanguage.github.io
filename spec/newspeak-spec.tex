...
\title{Newspeak Programming Language Draft Specification Version 0.106}
\author{Gilad Bracha}
\begin{document}
\maketitle
\tableofcontents
...
Here is the lexical grammar for Newspeak. It is a PEG (Parser Expression Grammar) with no distinction between scanning and parsing. The grammar is based on the implementation, which uses the parser combinator library described in \cite{bracha07a}.
...
\begin{newspeak}  
 ...
 comma $\rightarrow$  "";
 dot $\rightarrow$ ".";
 doublevbar $\rightarrow$ "$||$";
 equalSign $\rightarrow$ "=";
 hat $\rightarrow$ "\^{}";
 lbracket $\rightarrow$ "[";
 lcurly $\rightarrow$ "\{";
 lparen $\rightarrow$ "(";
 langleBracket $\rightarrow$ "$<$";
 mixinOperator $\rightarrow$  "$<:$";
 pound $\rightarrow$ "\#";
 rangleBracket $\rightarrow$ "$>$";
 rbracket $\rightarrow$ "]";
 rcurly $\rightarrow$ "\}";
 rparen $\rightarrow$ ")";
...
\begin{newspeak}
expression $\rightarrow$ [setterKeyword] cascadedMessageExpression;
\end{newspeak}
...
\begin{newspeak}
literal $\rightarrow$  pattern $|$ number $|$ symbol $|$ characterConstant $|$ string $|$ tuple;
\end{newspeak}
...
where
\begin{newspeak}
slotDecl $\rightarrow$ identifier;    
% In real life, we have an [type] at the end, but this is an override in the typed grammar, which
% part of teh optional type system. I deally, the types woukd be metdata and the tools would present them nicely.
codeBody $\rightarrow$ [temporaries]  statements;    
  
temporaries $\rightarrow$ slotDecls;
  
slotDecls $\rightarrow$ seqSlotDecls $|$ 
                      simSlotDecls;
...
\begin{newspeak}
pattern $\rightarrow$ "$<$", patternLiteral,  "$>$";
patternLiteral $\rightarrow$ wildcardPattern $|$ literalPattern $|$ keywordPattern;
\end{newspeak}
...
\begin{newspeak}
literalPattern $\rightarrow$ (whitespace $|$ comment)*  number $|$ symbol $|$ characterConstant $|$ string $|$ tuple;
\end{newspeak}
...
\begin{newspeak}
keywordPattern $\rightarrow$ kwPatternPair+; 
kwPatternPair $\rightarrow$ keyword, [kwPatternValue]. 
kwPatternValue $\rightarrow$ wildcardPattern $|$ literalPattern $|$ variablePattern $|$ nestedPatternLiteral; 
variablePattern $\rightarrow$ (whitespace $|$ comment)*  ( ('?'), id ); 
nestedPatternLiteral $\rightarrow$ (whitespace $|$ comment)*  pattern;
\end{newspeak}
...
\begin{newspeak}
objectLiteral $\rightarrow$ [identifier [keywordMsg]]  classBody;
\end{newspeak}
...
mixinAppSuffix $\rightarrow$ (mixinOperator  inheritanceClause)+
                      (dot $|$ classBody);
 %classExpression $\rightarrow$ classDeclaration $|$ mixinApplication.
 %classExpression $\rightarrow$ classPrefix, mixinSpecifier.
 %mixinSpecifier $\rightarrow$ classBody $|$ mixinApplication.
\end{newspeak}

An optional type parameter declaration may appear between the class name and the primary factory method pattern. It declares the formal type parameters of a generic class. For example, \code{class Map <[K, V]> new: capacity <Integer>} declares that \code{Map} has two type parameters, \code{K} and \code{V}. These names are in scope within the class body and may be used in type annotations on slots, method parameters, and return types.
...
%inheritanceClause $\rightarrow$ inheritanceClauseReceiver, message opt.
%inheritanceClauseReceiver $\rightarrow$ parenthesizedExpression |  inheritancePrefix opt, unarySelector.
...
% \begin{newspeak}
% classPrefix =  ( ( \#class),    (identifier, messagePattern, equalSign,  superclassClause opt) $|$
%               ( ( \#class),    (identifier, empty, equalSign,  superclassClause opt).   
%\end{newspeak}
...
% classSpecifier $\rightarrow$ classHeader, sideDecl, classSideDecl opt.
\end{newspeak}
where
...
%classHeader $\rightarrow$  lparen  classComment opt 
...
{\it
Nested class declarations, lazy slot declarations and method declarations may appear in any order on the instance side. The recommended style is to group all nested classes first, then all lazy slots, then all methods, but the grammar does not enforce this. Tools (class browsers, pretty printers) are free to display members in canonical order regardless of their source-text order.
}

 
 {\it
 The use of parentheses as delimiters for the sides and initializer derives from the syntax of Self, via certain Smalltalk dialects of nordic heritage. Perhaps they will  someday be replaced by curly braces, in conformance with widespread custom. sniff.
 }
...
slotDef $\rightarrow$  [accessModifier] slotDecl
     [((equalSign $|$ ''::='') expression dot) $|$ dot];
\end{newspeak}

{\it
The trailing dot is mandatory after an initializer and optional after an uninitialized slot, so all of  \code{$|$ a $|$}, \code{$|$ a. $|$}, \code{$|$ a = 1. b. c $|$} are well-formed slot bars.
}
...
lazySlotDecl $\rightarrow$ ((lazyModifier [accessModifier]) $|$ (accessModifier lazyModifier))
                      slotDecl (equalSign $|$  ''::='') expression dot;
\end{newspeak}

A lazy slot declaration has the form \code{lazy $m ::= e.$} or \code{lazy $m = e.$}. The token \code{lazy} may optionally be preceded {\em or} followed by an access modifier; both \code{lazy public $m = e.$} and \code{public lazy $m = e.$} are accepted, with identical meaning.
